...
export default function AutomatizarColetas() {
  ...
  return (
    <div className="container-buttons flex flex-row gap-4">
        ...
        <button className="retro-button text-lg blue-button" onClick={() => automatizarColetas("X", false)} disabled={score < precoAutomatizarColetas.X || statusAutoButtons.X}><ArrowPathIcon className="w-15 h-5 text-green-300" /> X</button>
        <button className="retro-button text-lg green-button" onClick={() => automatizarColetas("Y", false)} disabled={score < precoAutomatizarColetas.Y || statusAutoButtons.Y}><ArrowPathIcon className="w-15 h-5 text-green-300" /> Y</button>
    </div>
  );
}
